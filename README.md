...
- **壁纸星标收藏**：★/☆ 按钮收藏壁纸，收藏项置顶排序，localStorage 持久化

### 🎬 壁纸场景录制 — Wallpaper Engine 动态壁纸转 MP4

- **desktopCapturer 录制**：捕获 Wallpaper Engine 窗口 → MediaRecorder 录制 15 秒（VP8/WebM） → FFmpeg 转码 H.264 MP4
- **录制预览窗口**：全屏模态框含实时视频预览 + 状态胶囊 + 进度条 + 取消按钮
- **自动分辨率检测**：从屏幕 `displayBounds` 获取实际分辨率，不再硬编码 1920×1080
- **硬件编码自动检测**：NVIDIA nvenc → Intel QSV → AMD AMF → libx264 软件回退
- **开箱即用**：ffmpeg.exe 已打包到 `build/tools/`，无需用户额外安装
- **解包/录制双模式**：壁纸选择器顶部切换，解包为默认，录制为手动切换

### 🎞 热评卡片视频背景 — 壁纸视频导入

- **导入图片/视频**：一键导入本地图片或视频作为热评卡片背景
- **视频裁切对话框**：Canvas 实时预览 + 拖拽定位 + 缩放滑块（1x–3x），3:4 比例裁切
- **壁纸视频选择**：复用壁纸选择器，点击壁纸自动提取视频（MP4 直读 / PKG 解包）
- **壁纸场景录制**：多层纹理壁纸一键录制 15 秒视频，自动裁切应用
- **视频存储**：IndexedDB 存 blob（大文件），localStorage 存元数据，图片/视频互斥
...
| 均衡器 EQ | ❌ 无 | ✅ 6 频段 + 12 预设 |
| 声境预设 | ❌ 无 | ✅ 3D 地形渲染 |
| 壁纸录制 | ❌ 无 | ✅ desktopCapturer + FFmpeg |
| 热评视频背景 | ❌ 无 | ✅ 壁纸视频导入 + 裁切 |
| 壁纸星标收藏 | ❌ 无 | ✅ ★/☆ + 置顶排序 |
| 海啸/龙卷风/行星预设 | ❌ 无 | ✅ GLSL 着色器 |
