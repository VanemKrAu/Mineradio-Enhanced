...
function resolveRcedit(projectDir) {
  ...
  if (!hit) throw new Error('No usable rcedit executable was found for Mineradio-Enhanced icon injection.');
  return hit;
}
...
module.exports = async function afterPack(context) {
  ...
  const appName = context.packager.appInfo.productFilename || 'Mineradio-Enhanced';
  ...
  if (!fs.existsSync(exePath)) throw new Error(`Mineradio-Enhanced executable was not found: ${exePath}`);
  if (!fs.existsSync(iconPath)) throw new Error(`Mineradio-Enhanced icon was not found: ${iconPath}`);

  const version = context.packager.appInfo.version;
  console.log(`  • injecting Mineradio-Enhanced resources  rcedit=${rceditPath}`);
  execFileSync(rceditPath, [
    exePath,
    '--set-icon', iconPath,
    '--set-version-string', 'FileDescription', 'Mineradio-Enhanced',
    '--set-version-string', 'ProductName', 'Mineradio-Enhanced',
    '--set-version-string', 'CompanyName', 'Mineradio-Enhanced',
