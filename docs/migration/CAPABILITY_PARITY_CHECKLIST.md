...
- [ ] QQ 支持 search。  — P4.5/A6 已落地（jsososo `qq.api("search", {key,pageNo,pageSize,t:0})`），body.list 字段差异（songmid/songname/singer/albumname/interval）已 mapper 字节对齐；待 B1 QQ cookie 注入后端到端验证。
...
- [ ] 跨源换源逻辑在 sidecar service 层完成。  — 当前 Provider Adapter 已存在，但跨源 source-switching fallback 还未实现；待 source-switching slice 处理。
