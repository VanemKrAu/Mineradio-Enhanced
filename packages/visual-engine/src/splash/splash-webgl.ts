export const SPLASH_VERTEX_SHADER = String.raw`attribute vec2 aPosition;
varying vec2 vUv;
void main(){
...
export const SPLASH_FRAGMENT_SHADER = String.raw`precision highp float;
varying vec2 vUv;
...
export type SplashWebglOptions = Record<string, never>;

export function createSplashWebgl(canvas: HTMLCanvasElement, _opts: SplashWebglOptions = {}): SplashWebglHandle {
	const fail: SplashWebglHandle = {
		ok: false,
		render() {},
		resize() {},
		dispose() {},
	...
	const vertexShader = compile(gl.VERTEX_SHADER, SPLASH_VERTEX_SHADER);
	const fragmentShader = compile(gl.FRAGMENT_SHADER, SPLASH_FRAGMENT_SHADER);
